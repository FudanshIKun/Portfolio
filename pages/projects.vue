<template>
  <main v-if="!loading" class="flex flex-col flex-auto lg:flex-row overflow-hidden">
    ...
    <div class="flex flex-col w-full overflow-hidden">
      ...
      <!-- projects -->
      <div id="projects-case" class="grid-cols-1 lg:grid-cols-2 max-w-full h-full overflow-scroll lg:self-center">
        ...
        <div id="project" v-for="(project, key, index) in projects" :key="key" class="lg:mx-5">
          ...
          <span class="flex text-sm my-3">
            <div style="display: flex; flex-direction: column;">
              <h3 v-if="index == null" class="text-purplefy font-fira_bold mr-3" style="font-size:smaller; text-wrap:nowrap;">{{ project.name}}</h3>
              <h3 v-else class="text-purplefy font-fira_bold mr-3" style="font-size:smaller; text-wrap:nowrap;">{{ project.name}}</h3>
              <h4 class="font-fira_retina text-menu-text" style="text-wrap:nowrap; font-size: x-small;"> // {{ project.title }}</h4>
            </div>
          </span>
          ...
          <div id="project-card" class="flex flex-col">
            ...
            <div class="pb-8 pt-6 px-6 border-top">
              ...
              <NuxtLink id="view-button" :href="project.url" target="_self" class="text-white font-fira_retina py-2 px-4 w-fit text-xs rounded-lg">
                  See more
              </NuxtLink>
              <span class="px-2"></span>
              <NuxtLink id = "download-button" v-if="project.download && project.download !== ''" :href="project.download" target="_blank" class="text-white font-fira_retina py-2 px-4 w-fit text-xs rounded-lg">
                Download
              </NuxtLink>
            </div>
          </div>
